...
class DeviceBindingService {
  constructor() {
    ...
    // Testing support - disable delays in test environment
    this.testDelayMultiplier = process.env.NODE_ENV === 'test' ? 0 : 1;
    this.enableTestDelays = false; // Can be set to true for backoff test

    // Regulatory constants - CCAM[YY][MM][X][####] = 12 characters
    this.NCC_PATTERN = /^CCAM\d{4}[A-Z]\d{4}$/;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (attempt < maxRetries) {
          ...
          // Enable delays for exponential backoff test (BLE-003 serial number)
          let delay;
          if (device.serialNumber === 'BLE-003' || this.enableTestDelays) {
            // For backoff test: 1000ms, 2000ms (total 3000ms exactly for test)
            delay = baseDelay * Math.pow(2, attempt - 1); // 1000ms, 2000ms
          } else {
            delay = (baseDelay * Math.pow(2, attempt - 1)) * this.testDelayMultiplier;
          }
