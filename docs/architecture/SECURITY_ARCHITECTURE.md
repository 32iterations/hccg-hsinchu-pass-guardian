...
const securityMiddleware = {
  ...
  encryption: 'TLS 1.3 + Application Layer Field Encryption'
}
```
...
| 安全領域 | 等級 | 說明 |
|---------|------|------|
| **網路安全** | A+ | Cloudflare Enterprise DDoS 防護、WAF、TLS 1.3 |
| **身份驗證** | A+ | FIDO2/Passkeys、多因素認證、JWT RS256、Firebase Auth |
| **資料加密** | A+ | AES-256-GCM、Argon2id/bcrypt、應用層欄位加密 |
| **存取控制** | A | RBAC、Row Level Security |
| **監控審計** | A+ | 24/7 監控、審計日誌保存 ≥6 個月 |
| **合規性** | A+ | 個資法、資通安全管理法、ISO 27001 認證雲端服務 |
...
### 資料處理架構說明
- **非端到端加密架構**：系統採用 TLS 1.3 傳輸加密 + 磁區/KMS 靜態加密 + AES-256-GCM 欄位級加密的三層保護。由於需要在伺服器端進行定位計算、地理圍欄判斷、警報觸發等運算，因此不採用端到端加密（E2EE）架構。
- **最小化可見資料原則**：僅在必要運算時解密相關欄位，並強化審計追蹤。

### 跨境資料處理合規
- **主要部署於台灣**：系統主要基礎設施位於台灣，可選擇性於歐盟節點建立備援或特定工作負載。
- **歐盟處理者義務**：當使用歐盟雲端服務時，要求供應商以資料處理者身分遵循 GDPR 第 28 條義務，並簽訂資料保護附約（DPA）與標準契約條款（SCC）。
- **禁用區域管控**：明確禁止將資料存放或傳輸至中國大陸地區，符合政府機關資安規範。

### 密碼學實作
- **密碼雜湊**：優先採用 Argon2id（記憶體困難函數），備選 bcrypt with cost factor 12
- **對稱加密**：AES-256-GCM 提供認證加密（AEAD）
- **金鑰管理**：採用 KMS/HSM 進行金鑰生命週期管理，實施金鑰版本控制與定期輪替

### 認證機制演進
- **抗釣魚優先**：預設採用 FIDO2/WebAuthn Passkeys，提供硬體層級的抗釣魚保護
- **短期 Token 策略**：Access Token 15 分鐘有效期，Refresh Token 7-30 天並實施旋轉機制
- **即時撤銷能力**：透過 Redis 黑名單機制實現 Token 即時撤銷

---

## 🔍 審查準備文件清單

為確保順利通過政府機關資安審查，建議準備以下文件：

### 必要文件
1. **雲端服務商 ISO/IEC 27001 (CNS 27001) 證書** - 證明符合政府採購要求
2. **資料保護附約 (DPA) 與標準契約條款 (SCC)** - 跨境傳輸合法性文件
3. **不使用中國大陸地區雲端服務聲明書** - 符合資安法規定
4. **個資保護影響評估報告 (PIA)** - 證明已評估隱私風險
5. **審計日誌保存政策文件** - 證明符合 ≥6 個月保存要求

### 技術證據
1. **滲透測試報告** - 第三方安全評估結果
2. **弱點掃描報告** - 定期安全檢測紀錄
3. **DDoS 防護服務合約** - Cloudflare Enterprise 等級證明
4. **備援與災難復原計畫** - RTO/RPO 承諾文件
5. **事件應變計畫** - 資安事件處理流程

### 合規證明
1. **資通安全責任等級評估表** - 確認系統等級（A-E 級）
2. **個資法第 27 條安全措施檢核表** - 11 項安全措施落實證明
3. **OWASP Top 10 風險評估報告** - 依風險設計的證據
4. **資安政策與程序文件** - ISMS 相關文件

---

**安全聲明**: 本系統的安全架構定期接受第三方安全審計，並持續根據最新威脅情報進行更新。所有安全宣稱均基於實際部署的技術控制措施。

*文檔版本: v2.0.0 | 最後更新: 2025-01-21*
